		...
		<div class="card ad_card mb-4">
			<div class="card-body">
				...
				<form method="post" action="ad_list_notice" id="list">
									...
									<tr style="cursor: pointer;">
										...
										<td class="col-4">
											<c:forEach var="i" begin="1" end="${vo.indent }">
												...
												<c:if test="ad_detail_notice?id=${vo.id }?${vo.title}=${vo.title}">${vo.title}</c:if>
												</span>&nbsp;&nbsp;
											</c:forEach>
											<a href="ad_detail_notice?id=${vo.id }&curPage=${notice.curPage }">${vo.title }</a>
										</td>
					...
					<button class="btn btn-outline-secondary" id="delBtn">삭제</button>
...
$(function(){
	...
	$("#delBtn").click(function(){
		...
		$.ajax({
			url : "${pageContext.servletContext.contextPath}/delNotice",
			type : "POST",
		    data: {"noArr":noArr},
			success : function() {
				alert("success");
