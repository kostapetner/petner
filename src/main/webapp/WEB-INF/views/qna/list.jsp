...
<style>
.q_list {background: #f1f1f1;
    border-bottom: 1px solid #ccc;}
    .q_list .text {cursor: pointer;
    background: #fff;
    height: 40px;
    padding: 20px;
    display: flex;
    align-items: center;justify-content: space-between;}
    .q_list .hide{display:none;padding: 30px 20px;
    }
    .q_list .text svg {transform: rotate(180deg);}
</style>
<body>	
<h2>QNA</h2>
...
<div class="questions_list">
    <ul>
        <li class="q_list">
            <div class="text">질문1<i class="fa-solid fa-chevron-up"></i></div>
            <ul class="hide">
                <li>질문답변 !!!!!!</li>
            </ul>
        </li>
        ...
        <li class="q_list">
            <div class="text">질문2<i class="fa-solid fa-chevron-up"></i></div>
            <ul class="hide">
                <li>질문답변 !!!!!!</li>
            </ul>
        </li>
			...
			<ul class="search_box">
				<li>
					<select name="search" class="search">
						<option class="option_box" value="all" ${qna.search eq 'all' ? 'selected' : '' }>전체</option>
						<option class="option_box" value="title" ${qna.search eq 'title' ? 'selected' : '' }>제목</option>
						<option class="option_box" value="content" ${qna.search eq 'content' ? 'selected' : '' }>내용</option>
						<option class="option_box" value="writer" ${qna.search eq 'writer' ? 'selected' : '' }>작성자</option>
					</select>
				</li>
				<li><input value="${qna.keyword }" type="text" name="keyword" class="w-px300" /></li>
				...
				<div class="box2">
					<h2>
						<c:forEach var="i" begin="1" end="${vo.indent }">
							...
							<c:if test="detail_qna?id=${vo.id }?${vo.title}=${vo.title}">${vo.title}</c:if></span>&nbsp;&nbsp;
						</c:forEach>
...
<div class="btnSet">
	...
	<!-- step : 지정하지 않아도 디폴트 1 -->
	<c:forEach var="no" begin="${qna.beginPage }" end="${qna.endPage }" step="1">
